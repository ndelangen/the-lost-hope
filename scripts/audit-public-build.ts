...
async function audit(): Promise<void> {
  ...
  assert(PUBLIC_PAGE_DESCRIPTORS.length === 374, 'The public-page contract must contain 374 pages')
  ...
  assert(
    ...
    `Expected 374 sitemap pages, found ${builtPaths.length}`,
  )
  assert(
  ...
  assert(
    ...
    `Expected 374 sitemap URLs, found ${sitemapUrls.length}`,
  )
  assert(
  ...
  for (const page of PUBLIC_PAGE_DESCRIPTORS) {
    ...
    if (page.entity?.kind === 'location') {
      ...
      expectedSectionOrder.push('destination-map')
      assert(
  ...
  assert(socialFiles.length === 374, `Expected 374 social images, found ${socialFiles.length}`)
