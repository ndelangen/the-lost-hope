...
async function audit(): Promise<void> {
  ...
  assert(PUBLIC_PAGE_DESCRIPTORS.length === 349, 'The public-page contract must contain 349 pages')
  ...
  assert(
    ...
    `Expected 349 sitemap pages, found ${builtPaths.length}`,
  )
  assert(
  ...
  assert(
    ...
    `Expected 349 sitemap URLs, found ${sitemapUrls.length}`,
  )
  assert(
  ...
  assert(socialFiles.length === 349, `Expected 349 social images, found ${socialFiles.length}`)
